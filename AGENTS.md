...
- **Never annotate a closure in `_primitives.py` that gets re-defined on every call and closes over a live JAX tracer** (e.g. `_make_quax_branch`/`flat_quax_call` inside `cond_quax`) — `beartype.claw` decorates any annotated function it sees, and beartype's own `is_object_blacklisted` cache (`@callable_cached`, unbounded, never evicted) permanently pins that specific closure object — and everything it closes over, including live JAX tracers — for the rest of the process. `@no_type_check` alone does NOT prevent this (claw decides whether to decorate purely from annotation presence, before `@no_type_check` is ever consulted) — the fix is to strip the closure's parameter/return annotations entirely, not just mark it `@no_type_check`. Symptom: `JAX_CHECK_TRACER_LEAKS=1` failures with "Leaked trace" errors that trace back into beartype's internals. (`qfun` inside `jit_quax` is a re-defined-per-call closure too and stays fully annotated, just `@no_type_check`'d — it closes over only a weakref and a treedef, no live tracers, so being pinned by the cache is a harmless leaked-object, not a tracer leak. Don't copy that as a general pattern without checking what the closure actually holds.) **Fixed upstream in [beartype/beartype#673](https://github.com/beartype/beartype/issues/673) (commit `6c0cec9360c`, in `beartype>=0.23.0rc0`) — not yet in a stable beartype release, and excluded by `bearshape`'s own `beartype<0.23` pin. Revert this workaround (restore full annotations, drop the `@no_type_check` markers) once both a stable fixed beartype and a `bearshape` release supporting it are available.**
