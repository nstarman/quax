...
class _QuaxTrace(core.Trace[_QuaxTracer]):
    ...
    def process_custom_jvp_call(
        ...
        *,
        ...
        # Avoid intermediate list, directly flatten
        in_leaves, in_treedef = jtu.tree_flatten([self.to_value(t) for t in tracers])
...
def _custom_jvp_jvp_wrap(tag, in_treedef, *in_primals_and_tangents):
    ...
    in_tangent_values = jtu.tree_unflatten(in_treedef, in_primals_and_tangents[split:])
    ...
    with core.take_current_trace() as parent_trace:
        ...
        with core.set_current_trace(trace):
            ...
            # Zip once and iterate together to avoid double iteration
            pairs = list(zip(out_primal_values, out_tangent_values))
            out_primal_values2 = [
                p.materialise() if p.__class__ != t.__class__ else p for p, t in pairs
            ]
            out_tangent_values2 = [
                t.materialise() if p.__class__ != t.__class__ else t for p, t in pairs
            ]
            del out_tracers
        del trace, in_tracers
